...
export default class {
    ...
    get translations(): DemanaLocaleTranslationDto {
        try {
            ...
            throw new Error(`Failed to get translations for locale codes: ${(exception as Error).message} [current: ${this.locale}, default: ${this.defaultLocale}]`)
        }
    }
    ...
    private parseLocaleCode(value: string): string | null {
        return this.availableLocaleCodes.find(localeCode => value.toLowerCase().includes(localeCode.toLowerCase())) || null
    }

    translate = (key: string, context: Record<string, string> = {}): string => {
        try {
            const rawTranslation = key.split('.').reduce(
                (
                    translation: undefined | DemanaLocaleTranslation | string,
                    keyPart
            ...
            throw new Error(`Failed to translate key '${key}': ${(exception as Error).message}'`, { cause: exception })
        }
    }
}