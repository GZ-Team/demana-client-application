...
export function getBrowserWindowByProcessWebContents(
  processWebContents: WebContents
): BrowserWindow {
  const process = BrowserWindow.fromWebContents(processWebContents);

  if (!process) {
    throw new Error(
      `the process for the provided WebContents with id "${processWebContents.id}" was not found.`
    );
  }
